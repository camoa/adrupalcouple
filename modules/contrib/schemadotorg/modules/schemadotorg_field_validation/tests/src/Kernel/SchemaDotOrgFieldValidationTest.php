 ...
 * @group schemadotorg
 */
class SchemaDotOrgFieldValidationTest extends SchemaDotOrgKernelEntityTestBase {
  ...
  public function testEntityDisplayBuilder(): void {
    ...
    $this->assertEquals('regex_field_validation_rule', $configuration['id']);
    $this->assertEquals('Schema.org: Global Location Number (GLN)', $configuration['title']);
    $this->assertEquals('/^\\d{13}$/', $configuration['data']['setting']);
    $this->assertEquals('schema_global_location_number', $configuration['field_name']);
    $this->assertEquals('Global Location Number (GLN) must be a 13-digit number.', $configuration['error_message']);
